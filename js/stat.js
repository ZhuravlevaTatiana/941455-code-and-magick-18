...
window.renderStatistics = function (ctx, names, times) {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    var columnX = CLOUD_X + COLUMN_GAP + COLUMN_WIDTH * 2.5 * i;
    ctx.fillStyle = '#000';
    ctx.fillText(currentPlayer, columnX, CLOUD_HEIGHT);
    ctx.fillText(Math.round(currentTime), columnX, CLOUD_HEIGHT - columnHeight - FONT_GAP * 2);
    if (currentPlayer === 'Вы') {
      ctx.fillStyle = 'rgba(255, 0, 0, 1)';
    } else {
      ctx.fillStyle = 'hsl(250, ' + Math.random() * 100 + '%, 50%)';
    }
    ctx.fillRect(columnX, CLOUD_HEIGHT - columnHeight - FONT_GAP, COLUMN_WIDTH, columnHeight);
  }
};
